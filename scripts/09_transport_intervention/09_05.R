################################################################################
##################################            ##################################
######### 09.05) RUN R5R TRAVEL-TIME COUNTERFACTUAL MATRIX SIMULATION ##########
##################################            ##################################
################################################################################
...
r5_directory <- processed_path("interventions", "transport_intervention", "r5r_core")

# Retrieving OD shapes file
...
r5r_output <- file.path(output_directory, "r5r_output_counterfactual.parquet")
...
# Setting departure time window (minutes)
time_window <- 60

# Setting percentile of the travel-time distribution to report (median)
percentiles <- 50

# Setting Monte Carlo draws per minute for frequency-based GTFS services
draws_per_minute <- 5
