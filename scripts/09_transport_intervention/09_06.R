################################################################################
##################################            ##################################
######### 09.06) COMPLETE AND OUTPUT COUNTERFACTUAL TRAVEL-TIME MATRIX #########
##################################            ##################################
################################################################################
...
# Retrieving raw r5r output (from 09_05)
r5r_input <- processed_path("interventions", "transport_intervention", "matrix", "r5r_output_counterfactual.parquet")
...
output_directory <- processed_path("interventions", "transport_intervention", "matrix")

# Setting output file path (long-form 343 x 343 grid with pair-level sample
# flag; downstream scripts pivot to matrix form and/or filter sample == 1 as
# needed)
matrix_full_output <- file.path(output_directory, "counterfactual_matrix_full.parquet")
